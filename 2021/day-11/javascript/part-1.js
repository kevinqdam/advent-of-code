...
const stepNTimes = n => grid => {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    flashes += pipe(stepped).to(count2D(octopus => (octopus === 0))).value;
  }

  return { stepped, flashes };
};

const main = text => pipe(text)
  .to(parse)
  .to(stepNTimes(100))
  .to(getFromObj('flashes'))
  .value;

console.log(main(example)); // 1656
console.log(main(input));   // 1749
