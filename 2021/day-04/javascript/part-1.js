...
const getFirstWinningBoard = function (boards, numbers) {
  const turnsRequiredToWin = boards.map(board => playBingo(board, [...numbers].reverse()));
  const minTurns = Math.min(...turnsRequiredToWin);
  const indexOfFirstWinningBoard = turnsRequiredToWin.indexOf(minTurns);

  return { turns: minTurns, board: boards[indexOfFirstWinningBoard] };
};
...
const main = function (input) {
  ...
  const { turns, board: markedBoard } = getFirstWinningBoard(boards, numbers);
  const winningNumber = numbers[turns - 1];
  
  return score(markedBoard, winningNumber);
};

console.log(`Result for example: ${main(example)}`); // Result for example: 4512
console.log(`Result for input: ${main(input)}`);     // Result for input: 22680
